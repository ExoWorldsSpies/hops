...
class Observatory:
    ...
    def lt(self, time_obj):
        ...
        if self.time_zone < 0:
            return (time_obj - DTime(hours=np.abs(self.time_zone))).iso()
        else:
            return (time_obj + DTime(hours=np.abs(self.time_zone))).iso()

    def lst(self, time_obj):
        _request_time(time_obj)
        return time_obj.gmst() + self.longitude
    ...
    def target_azimuth_altitude(self, target, time_obj):
        ...
        ha = Degrees(self.lst(time_obj).deg() - target.ra.deg())
    ...
    def target_horizon_crossings(self, target, time_obj_1, window):
        ...
        test_alt = _target_over_horizon(target, JD(jd1))
        test_jd = jd1

        for jd in np.arange(jd1 + 0.007, jd2 + 0.0001, 0.007):

            new_test_alt = _target_over_horizon(target, JD(jd))
            new_test_jd = jd
            ...
            if test_alt * new_test_alt < 0:
                ...
                if test_alt > 0:
                    crossings.append(
                        [JD(test_jd + (0.007 * abs(test_alt / new_test_alt)) / (1 + abs(test_alt / new_test_alt))), 'set']
                    )
                else:
                    crossings.append(
                        [JD(test_jd + (0.007 * abs(test_alt / new_test_alt)) / (1 + abs(test_alt / new_test_alt))), 'rise']
                    )
    ...
    def target_altitude_crossings(self, target, time_obj_1, window, limit_altitude):
        ...
        test_alt = _target_over_altitude(target, JD(jd1))
        test_jd = jd1

        for jd in np.arange(jd1 + 0.007, jd2 + 0.0001, 0.007):

            new_test_alt = _target_over_altitude(target, JD(jd))
            new_test_jd = jd
    ...
    def periodic_events_visibility(self, target, time_obj_1, window, ref_epoch, ref_epoch_format, period, duration,
        ...
        t1 = target.convert_to_bjd_tdb(time_obj_1.jd(), time_format='JD_UTC')
        t2 = t1 + window
        ...
        while t + duration < t2:
            e1 = JD(target.convert_to_jd_utc(t, 'BJD_TDB'))
